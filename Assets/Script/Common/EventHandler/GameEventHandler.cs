...
{
    List<eGameEventType> handling_event_type_list = new List<eGameEventType>();

    // 실제로 이벤트를 받을 자식클래스(game object 등)에서 반드시 override 되어야 함.
    public abstract void HandleGameEvent(GameEvent ge);

    public void RegisterGameEvent(eGameEventType event_type, GameEventHandler handler = null)
    {
    ...
    public void UnregisterGameEvent(eGameEventType event_type, GameEventHandler handler = null)
    {
        if (null == handling_event_type_list)
            return;
...
{
    public eGameEventType event_type;
    ...
    public GameEvent(eGameEventType in_eventType, bool in_use_memory_stream = false)
    {
        Init(in_eventType, in_use_memory_stream);
    }

    public GameEvent(eGameEventType in_eventType, List<System.Action> in_Callback_List, bool in_use_memory_stream = false)
    {
    ...
    public GameEvent(eGameEventType in_eventType, System.Action in_Callback, bool in_use_memory_stream = false)
    {
    ...
    protected void Init(eGameEventType in_eventType, bool in_use_memory_stream = false)
    {
...
{
    static public Dictionary<eGameEventType, List<GameEventHandler>> game_event_handlers = new Dictionary<eGameEventType, List<GameEventHandler>>();
    ...
    static public void RegisterHandler(eGameEventType event_type, GameEventHandler handler)
    {
    ...
    static public void UnregisterHandler(eGameEventType event_type, GameEventHandler handler)
    {
        if (null == game_event_handlers)
            return;
        if (game_event_handlers.ContainsKey(event_type) == false)
